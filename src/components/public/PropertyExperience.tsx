...
export const PropertyExperience: React.FC<PropertyExperienceProps> = ({
  ...
  return (
    <div className="w-full bg-[#FAF9F6] py-10 sm:py-14 border-b border-stone-200/80 my-4">
      <div className="max-w-5xl mx-auto px-4 sm:px-8 lg:px-12 space-y-8 sm:space-y-12">
        {/* Header Block */}
        <div className="max-w-2xl space-y-2">
          <p className="text-[10px] sm:text-xs font-mono uppercase tracking-[0.2em] text-stone-500 font-normal">
            DESIGNED FOR BETTER LIVING
          </p>
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-normal text-stone-900 tracking-tight leading-[1.2]">
          ...
          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            {experienceItems.map((item, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="p-4 sm:p-5 rounded-2xl bg-white/80 border border-stone-200/80 shadow-2xs space-y-2.5"
                >
                  ...
                  <div>
                    <h3 className="text-xs sm:text-sm font-medium text-stone-900">
                      {item.title}
                    </h3>
                    <p className="text-[11px] sm:text-xs text-stone-500 font-normal leading-relaxed mt-0.5">
                      {item.desc}
                    </p>
                  </div>
                </div>
