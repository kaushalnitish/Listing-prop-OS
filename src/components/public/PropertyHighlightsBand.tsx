...
export const PropertyHighlightsBand: React.FC<PropertyHighlightsBandProps> = ({
  ...
  return (
    <div className="w-full bg-zinc-950 text-stone-100 py-10 sm:py-14 my-6 sm:my-10 border-y border-zinc-800/80 shadow-inner">
      <div className="max-w-5xl mx-auto px-4 sm:px-8 lg:px-12 space-y-8">
        {/* Header Eyebrow */}
        <div>
          <h2 className="text-[10px] sm:text-xs font-mono uppercase tracking-[0.22em] text-stone-400 font-normal">
            PROPERTY HIGHLIGHTS
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-0 lg:divide-x lg:divide-zinc-800/90">
          {highlights.map((item, idx) => {
            const Icon = item.icon;
            return (
              <div
                key={idx}
                className="flex flex-col space-y-2.5 lg:px-6 first:lg:pl-0 last:lg:pr-0"
              >
                ...
                <div>
                  <h3 className="text-xs sm:text-sm font-medium text-stone-100 tracking-tight">
                    {item.title}
                  </h3>
                  <p className="text-xs text-stone-400 font-normal leading-relaxed mt-1 line-clamp-3">
                    {item.description}
                  </p>
                </div>
              </div>
